...
class UGCScriptListScreen extends ConsumerWidget {
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: scriptsAsync.when(
          ...
          error: (e, _) => Center(
            child: Text('加载失败：$e', style: const TextStyle(color: AppColors.danger)),
          ),
  ...
  void _createNew(BuildContext context) {
    Navigator.of(context).push(
      MaterialPageRoute(builder: (_) => const UGCScriptEditorScreen()),
    );
  }

  void _edit(BuildContext context, UGCScript script) {
    Navigator.of(context).push(
      MaterialPageRoute(builder: (_) => UGCScriptEditorScreen(editingScript: script)),
    );
  }

  Future<void> _delete(BuildContext context, WidgetRef ref, UGCScript script) async {
    ...
    if (confirmed == true) {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('剧本已删除'), backgroundColor: AppColors.success),
        );
      }
    }
  }

  Future<void> _export(BuildContext context, WidgetRef ref, UGCScript script) async {
    try {
      final service = ref.read(ugcScriptServiceProvider);
      ...
      if (context.mounted) {
        showDialog(
          ...
          builder: (context) => AlertDialog(
            backgroundColor: AppColors.surface,
            title: const Text('导出成功', style: TextStyle(color: AppColors.silver)),
            content: Column(
              mainAxisSize: MainAxisSize.min,
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('剧本已导出到：', style: TextStyle(color: AppColors.textMuted)),
            ...
            actions: [
              TextButton(
                ...
                child: const Text('复制路径', style: TextStyle(color: AppColors.coolAccent)),
              ),
              TextButton(
                onPressed: () => Navigator.of(context).pop(),
                child: const Text('关闭', style: TextStyle(color: AppColors.silver)),
              ),
            ],
          ),
        );
      ...
      builder: (context) => AlertDialog(
        ...
        content: const SingleChildScrollView(
          child: Column(
            ...
            mainAxisSize: MainAxisSize.min,
            children: [
              Text('什么是 UGC 剧本？', style: TextStyle(color: AppColors.coolAccent, fontWeight: FontWeight.bold)),
              SizedBox(height: 4),
              Text(
              ...
              SizedBox(height: 12),
              Text('如何创作？', style: TextStyle(color: AppColors.coolAccent, fontWeight: FontWeight.bold)),
              SizedBox(height: 4),
              Text(
                '1. 点击"创建剧本"按钮\n2. 填写剧本标题和作者\n3. 添加对话，设置说话者和内容\n4. 保存即可',
              ...
              SizedBox(height: 12),
              Text('如何分享？', style: TextStyle(color: AppColors.coolAccent, fontWeight: FontWeight.bold)),
              SizedBox(height: 4),
              Text(
                '点击剧本卡片的"导出"按钮，将生成 JSON 文件，你可以分享给其他人。其他人可以通过"导入"功能使用你的剧本。',
                style: TextStyle(color: AppColors.textMuted, fontSize: 13, height: 1.5),
              ),
            ],
          ),
        ...
        actions: [
          TextButton(
            onPressed: () => Navigator.of(context).pop(),
            child: const Text('知道了', style: TextStyle(color: AppColors.coolAccent)),
          ),
        ],
      ),
    );
  ...
  Widget build(BuildContext context) {
    return Center(
      child: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
          Icon(Icons.edit_note, size: 80, color: AppColors.textMuted.withValues(alpha: 0.5)),
          const SizedBox(height: 16),
          const Text('还没有创作任何剧本', style: TextStyle(color: AppColors.textMuted, fontSize: 16)),
              ...
              PopupMenuButton<String>(
                ...
                itemBuilder: (context) => [
                  ...
                  const PopupMenuItem(
                    ...
                    child: Row(
                      children: [
                        Icon(Icons.upload, size: 18, color: AppColors.coolAccent),
                        SizedBox(width: 8),
