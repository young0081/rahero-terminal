...
- 👤 **个人信息**：名片、使用统计、收藏夹
- 🎨 **主题切换**：5套预设主题，动态配色
- 🖼️ **自定义头像**：Android/桌面端可从本地相册或文件选择头像，飞讯中实时生效
- 🎯 **新手引导**：7步图文引导，首次自动显示
...
- ⏳ **UGC导入导出收尾**：修复 `file_picker` 兼容性，恢复剧本文件导入导出
...
- 当前限制：受 `file_picker` 兼容性影响，UGC 文件导入/导出暂不可用；头像上传已开放

### 系统要求
- **Windows**: Windows 10/11 (64位) ✅ 已测试
- **Linux**: 未测试（代码已准备，需Linux环境构建）
- **Android**: Android 7.0+（API 24+）✅ 已完成 Release APK 构建
- **内存**: 4GB RAM（推荐8GB）
- **存储**: 200MB 可用空间
...
- 🖼️ **头像上传恢复**：Android/桌面端可选择本地图片作为飞讯头像
